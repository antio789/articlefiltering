...
def llm_prompt(string):
    response = generate(model='qwen40x2k', prompt=string,
                        options={ 'num_predict': 10000,"think": True})
...
article_prompt = read_file("prompts/reactor_prompt")
article_questions = read_json('prompts/q_articles.json')

articles_list = glob.glob("reactortxt/*.txt")
...
def process_questions(questionnaire, text, prompt):
    ...
    for section in questionnaire:
        if section.get("type") == "reactor":
            for q in section.get("questions"):
                logger.info(str(q.get("qid")) + " " + q.get("question"))
...
def run_reactorarticle(path):
    ...
    tempname = str(path).replace(".txt", "").replace("reactortxt/", "")

    name = 'reactoroutput/' + tempname + '.json'
    filtered.append({
